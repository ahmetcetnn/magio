  ...
  const onSubmit = async (data: LoginForm) => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        try {
          const r = await api.post('/users/refresh-token');
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center px-8 sm:px-12 lg:px-16 xl:px-20">
        ...
        <div className="space-y-6">
          ...
          <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
            <div>
              ...
              <input
                ...
                disabled={isLoading}
                className={`w-full pl-5 pr-6 pt-4 pb-4 border focus:outline-none focus:ring-2 ${errors.email ? 'focus:ring-red-500' : 'focus:ring-lime-500'} focus:border-transparent text-sm placeholder-gray-400 ${
                  isLoading ? 'bg-gray-100 cursor-not-allowed text-gray-500' : 'text-[#78778B] bg-white'
          ...
          <div className="mt-2.5">
            <button disabled={isLoading} className={`w-full flex items-center justify-center space-x-3 px-5 py-3.5 border border-gray-300 transition-colors duration-200 ${isLoading ? 'bg-gray-100 cursor-not-allowed' : 'hover:bg-gray-50'}`} style={{ borderRadius: '10px' }}>
